...
//		StreamInfo streamInfo = redisCatchStorage.queryPlayByStreamId(streamId);
		...
		if (!rtpInfo.getBoolean("exist")) {
			...
			String dstUrl = String.format("rtmp://%s:%s/convert/%s", "192.168.10.43", mediaInfo.getRtmpPort(),
					streamId );
			String srcUrl = String.format("rtsp://%s:%s/rtp/%s", "192.168.10.43", mediaInfo.getRtspPort(), streamId);
