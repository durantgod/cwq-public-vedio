...
public class StreamProxyServiceImpl implements IStreamProxyService {
    ...
    public void save(StreamProxyItem param, GeneralCallback<StreamInfo> callback) {
        ...
        if ("ffmpeg".equalsIgnoreCase(param.getType())) {
            ...
            dstUrl = String.format("%s://%s:%s/%s/%s", schemaForUri, "192.168.10.43", port, param.getApp(),
                    param.getStream());
        }else {
            dstUrl = String.format("rtsp://%s:%s/%s/%s", "192.168.10.43", mediaInfo.getRtspPort(), param.getApp(),
                    param.getStream());
        }
