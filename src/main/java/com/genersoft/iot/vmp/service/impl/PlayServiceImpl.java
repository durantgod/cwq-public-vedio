...
public class PlayServiceImpl implements IPlayService {
    ...
    private void snapOnPlay(MediaServerItem mediaServerItemInuse, String deviceId, String channelId, String stream) {
        ...
        if (mediaServerItemInuse.getRtspPort() != 0) {
            streamUrl = String.format("rtsp://192.168.10.43:%s/%s/%s", mediaServerItemInuse.getRtspPort(), "rtp", stream);
        } else {
            streamUrl = String.format("http://192.168.10.43:%s/%s/%s.live.mp4", mediaServerItemInuse.getHttpPort(), "rtp", stream);
        }
        String path = "snap";
    ...
    public void getSnap(String deviceId, String channelId, String fileName, ErrorCallback errorCallback) {
        ...
        if (inviteInfo != null) {
            if (inviteInfo.getStreamInfo() != null) {
                ...
                String streamUrl;
                if (mediaServerItemInuse.getRtspPort() != 0) {
                    streamUrl = String.format("rtsp://192.168.10.43:%s/%s/%s", mediaServerItemInuse.getRtspPort(), "rtp",  inviteInfo.getStreamInfo().getStream());
                }else {
                    streamUrl = String.format("http://192.168.10.43:%s/%s/%s.live.mp4", mediaServerItemInuse.getHttpPort(), "rtp",  inviteInfo.getStreamInfo().getStream());
                }
                String path = "snap";
                // 请求截图
                logger.info("[请求截图]: " + fileName);
